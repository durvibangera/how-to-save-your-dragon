/**
 * "How to Save Your Dragon" – 2D Top-Down Pixel Adventure
...
export class SiegeGame {
  constructor(container, onComplete) {
    ...
    // Pause menu state
    this._siegePaused = false;
    this._pauseMenuEl = null;
    this._chapterSelectEl = null;
  ...
  _setupControls() {
    this._keyDown = (e) => {
      // ESC for pause menu
      if (e.code === 'Escape') {
        if (this._chapterSelectEl) {
          this._closeChapterSelect();
          this._showSiegePauseMenu();
          return;
        }
        if (this._pauseMenuEl) {
          this._closeSiegePauseMenu();
          return;
        }
        this._showSiegePauseMenu();
        return;
      }
      if (this._pauseMenuEl || this._chapterSelectEl) return;
  ...
  _gameLoop() {
    ...
    // Skip updates when paused via ESC menu
    if (this._siegePaused) {
      this._draw();
      return;
    }
  ...
  _drawEnding(ctx) {
    ...
    if (this.endingPhase >= 5) {
      ...
      ctx.font = '7px monospace';
      ctx.fillStyle = '#ffd700';
      ctx.fillText('\u2661 How to Save Your Dragon \u2661', cx, cy + 56);
    }
  }
  ...
  _showSiegePauseMenu() {
    ...
    overlay.style.cssText = `
      ...
      display:flex;align-items:center;justify-content:center;
      pointer-events:all;z-index:20000;animation:fadeIn 0.3s ease;
    `;
    overlay.innerHTML = `
      <div style="
        background:linear-gradient(145deg, rgba(30,20,10,0.95), rgba(15,10,5,0.98));
        border:2px solid rgba(255,220,100,0.4);border-radius:12px;
        padding:2.5rem 3rem;min-width:320px;text-align:center;
        box-shadow:0 12px 40px rgba(0,0,0,0.8),inset 0 1px 0 rgba(255,255,255,0.1);
        font-family:'Cinzel',Georgia,serif;
      ">
        <h2 style="font-size:2rem;color:#ffeaa7;font-weight:700;margin:0 0 1.5rem;
          text-transform:uppercase;letter-spacing:0.1em;text-shadow:0 0 15px rgba(255,234,167,0.4);">
          Paused
        </h2>
        <button class="sp-btn" data-action="resume" style="
          display:block;width:100%;font-family:'Cinzel',Georgia,serif;font-size:1.05rem;font-weight:600;
          padding:14px 20px;margin-bottom:10px;border:1px solid rgba(255,220,100,0.3);
          background:rgba(139,69,19,0.4);color:#ffeaa7;border-radius:6px;cursor:pointer;
          transition:all 0.25s;letter-spacing:0.08em;text-transform:uppercase;
        ">Resume</button>
        <button class="sp-btn" data-action="chapter" style="
          display:block;width:100%;font-family:'Cinzel',Georgia,serif;font-size:1.05rem;font-weight:600;
          padding:14px 20px;margin-bottom:10px;border:1px solid rgba(255,220,100,0.3);
          background:rgba(139,69,19,0.4);color:#ffeaa7;border-radius:6px;cursor:pointer;
          transition:all 0.25s;letter-spacing:0.08em;text-transform:uppercase;
        ">Chapter Select</button>
        <button class="sp-btn" data-action="restart" style="
          display:block;width:100%;font-family:'Cinzel',Georgia,serif;font-size:1.05rem;font-weight:600;
          padding:14px 20px;margin-bottom:10px;border:1px solid rgba(255,220,100,0.3);
          background:rgba(139,69,19,0.4);color:#ffeaa7;border-radius:6px;cursor:pointer;
          transition:all 0.25s;letter-spacing:0.08em;text-transform:uppercase;
        ">Restart Level</button>
        <button class="sp-btn" data-action="home" style="
          display:block;width:100%;font-family:'Cinzel',Georgia,serif;font-size:1.05rem;font-weight:600;
          padding:14px 20px;margin-bottom:0;border:1px solid rgba(255,220,100,0.3);
          background:rgba(139,69,19,0.4);color:#ffeaa7;border-radius:6px;cursor:pointer;
          transition:all 0.25s;letter-spacing:0.08em;text-transform:uppercase;
        ">Back to Home</button>
      </div>
    ...
    // Hover effects
    overlay.querySelectorAll('.sp-btn').forEach(btn => {
      btn.addEventListener('mouseenter', () => {
        btn.style.background = 'rgba(139,69,19,0.7)';
        btn.style.borderColor = 'rgba(255,220,100,0.7)';
      });
      btn.addEventListener('mouseleave', () => {
        btn.style.background = 'rgba(139,69,19,0.4)';
        btn.style.borderColor = 'rgba(255,220,100,0.3)';
      });
    });
    ...
    this._pauseMenuEl = overlay;
  }

  _closeSiegePauseMenu() {
    if (this._pauseMenuEl) {
      this._pauseMenuEl.remove();
      this._pauseMenuEl = null;
    }
    this._siegePaused = false;
  }

  _showSiegeChapterSelect() {
    if (this._chapterSelectEl) return;
    this._siegePaused = true;

    const siegeChapters = [
      { id: 1,   icon: '\ud83d\udde1\ufe0f', name: 'Berk Outskirts', weapons: 'Sword' },
      { id: 2,   icon: '\ud83c\udf32', name: 'Forbidden Forest', weapons: 'Sword, Bow, Spear' },
      { id: 2.5, icon: '\ud83e\udde9', name: 'Restore the Dragon Eye', weapons: 'Sword, Bow, Spear' },
      { id: 3,   icon: '\ud83c\udfdd\ufe0f', name: 'Dragon Island', weapons: 'Sword, Bow, Spear' },
      { id: 3.5, icon: '\ud83d\udd25', name: 'Gauntlet of Fire', weapons: 'Sword, Bow, Spear, Halberd' },
      { id: 4,   icon: '\ud83d\udc80', name: "The Red Death's Nest", weapons: 'All Weapons' },
    ];
    ...
    let html = `<div style="
      ...
      border:2px solid rgba(255,220,100,0.4);border-radius:12px;
      padding:2rem 2.5rem;min-width:420px;max-width:600px;max-height:85vh;overflow-y:auto;
      box-shadow:0 12px 40px rgba(0,0,0,0.8),inset 0 1px 0 rgba(255,255,255,0.1);
      font-family:'Cinzel',Georgia,serif;
    ">
      <h2 style="font-size:1.8rem;color:#ffeaa7;font-weight:700;text-align:center;margin:0 0 1rem;
        text-transform:uppercase;letter-spacing:0.1em;text-shadow:0 0 15px rgba(255,234,167,0.4);">
        Chapter Select
      </h2>`;

    siegeChapters.forEach(ch => {
      html += `<button class="sch-btn" data-level="${ch.id}" style="
        display:flex;align-items:center;gap:12px;width:100%;
        font-family:'Cinzel',Georgia,serif;font-size:0.95rem;font-weight:600;
        padding:12px 16px;margin-bottom:6px;border:1px solid rgba(255,220,100,0.2);
        background:rgba(139,69,19,0.25);color:#ffeaa7;border-radius:6px;
        cursor:pointer;transition:all 0.25s;text-align:left;
      ">
        <span style="font-size:1.3rem;flex-shrink:0;width:32px;text-align:center;">${ch.icon}</span>
        <span style="flex:1;">
          <span style="display:block;margin-bottom:2px;">${ch.name}</span>
          <span style="display:block;font-family:'Crimson Pro',Georgia,serif;font-size:0.75rem;
            font-weight:400;color:rgba(255,234,167,0.5);letter-spacing:0.03em;text-transform:none;">
            ${ch.weapons}
          </span>
        </span>
      </button>`;
    ...
    html += `<button class="sch-back" style="
      display:block;width:100%;font-family:'Cinzel',Georgia,serif;font-size:0.95rem;font-weight:600;
      padding:12px 20px;margin-top:1rem;border:1px solid rgba(255,255,255,0.2);
      background:rgba(255,255,255,0.05);color:rgba(255,255,255,0.7);border-radius:6px;
      cursor:pointer;transition:all 0.25s;letter-spacing:0.08em;text-transform:uppercase;text-align:center;
    ">Back</button>`;
    html += '</div>';
    overlay.innerHTML = html;

    // Hover effects
    overlay.querySelectorAll('.sch-btn').forEach(btn => {
      btn.addEventListener('mouseenter', () => {
        btn.style.background = 'rgba(139,69,19,0.55)';
        btn.style.borderColor = 'rgba(255,220,100,0.5)';
        btn.style.transform = 'translateX(4px)';
      });
      btn.addEventListener('mouseleave', () => {
        btn.style.background = 'rgba(139,69,19,0.25)';
        btn.style.borderColor = 'rgba(255,220,100,0.2)';
    ...
    overlay.querySelectorAll('.sch-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        const levelId = parseFloat(btn.getAttribute('data-level'));
        this._closeChapterSelect();
        // Set weapons for the selected level
        const weapons = weaponsByLevel[levelId] || weaponsByLevel[1];
        this.player.hasWeapons = { ...weapons };
        this._loadLevel(levelId);
      });
    });

    overlay.querySelector('.sch-back').addEventListener('click', () => {
      this._closeChapterSelect();
      this._showSiegePauseMenu();
    });

    document.body.appendChild(overlay);
    this._chapterSelectEl = overlay;
  }

  _closeChapterSelect() {
    if (this._chapterSelectEl) {
      this._chapterSelectEl.remove();
      this._chapterSelectEl = null;
    }
    this._siegePaused = false;
  }
