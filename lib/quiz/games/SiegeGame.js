...
export class SiegeGame {
  ...
  _spawnWave(waveNum) {
    ...
    const spawnPoints = [
      ...
      { x: 780, y: 100 },   // top-right
      { x: 100, y: 760 },   // bottom-left
      { x: 780, y: 760 },  // bottom-right
      { x: 450, y: 100 },   // top-center
      { x: 100, y: 450 },   // left-center
      { x: 780, y: 450 },  // right-center
      { x: 250, y: 100 },   // top-left-mid
      { x: 650, y: 100 },   // top-right-mid
      { x: 250, y: 760 },   // bottom-left-mid
      { x: 650, y: 760 },   // bottom-right-mid
    ];
    ...
    switch (waveNum) {
      case 1: // Dragon Swarm: lighter mix of shields + ranged
        spawnWithEffect(spawnPoints[0].x, spawnPoints[0].y, 'shield_guard', 35, 4);
        spawnWithEffect(spawnPoints[1].x, spawnPoints[1].y, 'knight', 30, 4);
        spawnWithEffect(spawnPoints[4].x, spawnPoints[4].y, 'crossbow_archer', 20, 6);
        spawnWithEffect(spawnPoints[5].x, spawnPoints[5].y, 'slime', 18, 3);
        spawnWithEffect(spawnPoints[6].x, spawnPoints[6].y, 'archer', 18, 5);
        break;

      case 2: // RAGNAROK: captain + war_golem + elite mix, max lava
        ...
        spawnWithEffect(spawnPoints[1].x, spawnPoints[1].y, 'dark_knight', 75, 15);
        spawnWithEffect(spawnPoints[2].x, spawnPoints[2].y, 'fire_mage', 45, 13);
        spawnWithEffect(spawnPoints[3].x, spawnPoints[3].y, 'necromancer', 40, 8);
        spawnWithEffect(spawnPoints[5].x, spawnPoints[5].y, 'berserker', 32, 9);
        spawnWithEffect(spawnPoints[6].x, spawnPoints[6].y, 'crossbow_archer', 30, 11);
        this.lavaPoolMaxActive = 5;
        this.shakeAmount = 12;
        this.shakeTimer = 40;
        this._showHudMessage('⚠️ RAGNAROK! Drago Bludvist commands the final army! ⚠️');
        break;
    }
      ...
      if (Math.hypot(px - gx, py - gy) < 60) {
        if (g.requirePuzzle && !this.puzzleSolved) {
          ...
          if (this.wavesCleared) {
            ...
            this._showHudMessage('Enemies approach! Survive the waves to open the gate!');
            this._playSound('horn');
          } else {
  ...
  _drawRunePuzzle(ctx) {
    ...
    // Only draw runes in level 3.5 when puzzle is active or just solved
    if (this.level !== 3.5 || (!rp.active && !rp.solved)) return;
  ...
  _updateWaveSystem() {
    ...
    if (this.waveState === 'pause') {
      ...
      // Periodic health drops during waves (every 15 seconds, but NOT on last 2 waves)
      ...
      if (this.waveHealthDropTimer >= 900) { // 15 seconds at 60fps
        // NO health drops during waves 6 and 7 (last two)
      ...
      if (alive === 0) {
        ...
        if (this.currentWave >= this.totalWaves) {
          ...
          // Activate rune puzzle after wave 1  
          if (this.currentWave === 1 && this.runePuzzle && !this.runePuzzle.solved) {
          ...
          // Grant halberd after wave 1 is cleared
          if (this.currentWave === 1 && !this.player.hasWeapons.halberd) {
          ...
          // Health drops between waves (not on last 2 waves)
          if (this.currentWave < this.totalWaves - 1) {
  ...
  _drawHUD(ctx) {
    ...
    if (this.level === 3.5 && this.waveLevel) {
      ...
      if (this.waveState === 'active') {
        ...
        ctx.fillStyle = '#ffdd44';
        ctx.fillText('Approach the gate to begin...', this.width / 2, 67);
      }
