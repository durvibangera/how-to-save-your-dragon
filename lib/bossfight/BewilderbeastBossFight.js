...
export class BewilderbeastBossFight {
  ...
  _bindInput() {
    const kd = (e) => {
      ...
      if (["Space", "ShiftLeft", "ShiftRight", "KeyC",
    ...
    const md = (e) => {
      ...
      if (!this.ptrLocked && this.state !== "loading" && !this.paused
          && this.renderer && this.renderer.domElement && this.renderer.domElement.isConnected)
    ...
    const plc = () => {
      ...
      // Reset all keys and mouse buttons when pointer lock is lost to prevent stuck keys
      if (!this.ptrLocked) {
        this.keys = {};
        this.mb.l = false;
        this.mb.r = false;
    ...
    this._handlers = { kd, ku, mm, md, mu, plc, bl, cm, rs };
  }
  ...
  _buildUI() {
    ...
    ld.innerHTML = `
      <h1 style="font-family:'Cinzel',serif;color:#ffeaa7;font-size:clamp(2rem,5vw,3rem);font-weight:900;margin-bottom:18px;text-shadow:3px 3px 6px rgba(0,0,0,0.9),0 0 20px rgba(0,0,0,0.5);letter-spacing:0.05em;text-transform:uppercase;">FINAL BOSS FIGHT - PROVE YOUR WORTH</h1>
    ...
    cs.innerHTML = `<div style="text-align:center">
      ...
      <p style="font-family:'Crimson Pro',Georgia,serif;color:rgba(245,245,245,0.85);font-size:14px;margin-top:18px;text-shadow:2px 2px 4px rgba(0,0,0,0.9);letter-spacing:0.02em;">Mouse: Aim &nbsp;|&nbsp; WASD: Move &nbsp;|&nbsp; Space / C: Up / Down</p>
    ...
    <!-- Pause menu -->
    <div id="bf-pause" style="position:absolute;top:0;left:0;width:100%;height:100%;display:none;align-items:center;justify-content:center;background:rgba(0,0,0,0.75);backdrop-filter:blur(6px);pointer-events:auto;z-index:200">
      <div style="background:linear-gradient(145deg,rgba(30,20,10,0.95),rgba(15,10,5,0.98));border:2px solid rgba(255,220,100,0.4);border-radius:12px;padding:2.5rem 3rem;min-width:320px;text-align:center;box-shadow:0 12px 40px rgba(0,0,0,0.8),inset 0 1px 0 rgba(255,255,255,0.1)">
        <h2 style="font-family:'Cinzel',serif;font-size:2rem;color:#ffeaa7;font-weight:700;margin:0 0 1.5rem 0;text-transform:uppercase;letter-spacing:0.1em;text-shadow:0 0 15px rgba(255,234,167,0.4)">PAUSED</h2>
        <button id="bf-pause-resume" style="display:block;width:100%;font-family:'Cinzel',serif;font-size:1.05rem;font-weight:600;padding:14px 20px;margin-bottom:10px;border:1px solid rgba(255,220,100,0.3);background:rgba(139,69,19,0.4);color:#ffeaa7;border-radius:6px;cursor:pointer;transition:all .25s ease;letter-spacing:0.08em;text-transform:uppercase">Resume</button>
        <button id="bf-pause-restart" style="display:block;width:100%;font-family:'Cinzel',serif;font-size:1.05rem;font-weight:600;padding:14px 20px;margin-bottom:10px;border:1px solid rgba(255,220,100,0.3);background:rgba(139,69,19,0.4);color:#ffeaa7;border-radius:6px;cursor:pointer;transition:all .25s ease;letter-spacing:0.08em;text-transform:uppercase">Restart</button>
        <button id="bf-pause-home" style="display:block;width:100%;font-family:'Cinzel',serif;font-size:1.05rem;font-weight:600;padding:14px 20px;margin-bottom:0;border:1px solid rgba(255,220,100,0.3);background:rgba(139,69,19,0.4);color:#ffeaa7;border-radius:6px;cursor:pointer;transition:all .25s ease;letter-spacing:0.08em;text-transform:uppercase">Exit to Menu</button>
      </div>
    </div>`;
    ...
    style.textContent = `
      ...
      #bf-pause-resume:hover,#bf-pause-restart:hover,#bf-pause-home:hover{background:rgba(139,69,19,0.7)!important;border-color:rgba(255,220,100,0.7)!important;transform:translateY(-1px);box-shadow:0 4px 15px rgba(0,0,0,0.4)!important;}
    `;
    document.head.appendChild(style);
    this.ui.style = style;

    /* Store pause UI references */
    this.ui.pauseOverlay = document.getElementById("bf-pause");
    const resumeBtn = document.getElementById("bf-pause-resume");
    const restartBtn = document.getElementById("bf-pause-restart");
    const homeBtn = document.getElementById("bf-pause-home");
    
    if (resumeBtn) resumeBtn.onclick = () => this._togglePause();
  ...
  _togglePause() {
    ...
    if (this.paused) {
      // Show pause menu and exit pointer lock
      if (this.ui.pauseOverlay) this.ui.pauseOverlay.style.display = "flex";
      if (document.pointerLockElement) document.exitPointerLock();
    } else {
      // Hide pause menu and restore pointer lock
      if (this.ui.pauseOverlay) this.ui.pauseOverlay.style.display = "none";
  ...
  _loadProg(pct) {
    ...
    if (txt) {
      if (pct < 30) txt.textContent = "Are u ready??";
      else if (pct < 60) txt.textContent = "Get ready!";
      else if (pct < 95) txt.textContent = "Almost here.";
  ...
  dispose() {
    ...
    if (h.bl) window.removeEventListener("blur", h.bl);
